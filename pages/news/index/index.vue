	...
	<view class="content">
		...
		*  -->
		...
		<mix-advert 
			ref="mixAdvert" 
			:timedown="8" 
			imageUrl="/static/image/advert.jpg"
			:url="advertNavUrl"
		></mix-advert>
		<!-- #endif -->
				...
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
					<scroll-view 
						...
						<view v-for="(item, index) in tabItem.newsList" :key="index" class="news-item" @click="navToDetails(item)">
							<text :class="['title', 'title'+item.type]">{{item.desc}}</text>
							<view v-if="item.imgNum> 0" :class="['img-list', 'img-list3', item.imgNum+'' === 1 && item.type===3 ? 'img-list-single': '']">
								<view 
									v-for="index  of item.imgNum"  :key="index"
									:class="['img-wrapper', 'img-wrapper'+item.type, item.imgNum+'' === 1 && item.type===3 ? 'img-wrapper-single': '']"
								>
							{{index }}
	...
	export default {
		...
		computed: {
			advertNavUrl(){
				...
				return `/pages/news/details/details?data=${JSON.stringify(data)}`;
			} 
		},
		async onLoad() {
			// 获取屏幕宽度
		...
		methods: {
			...
			navToDetails(item){
				...
				uni.navigateTo({
					url: `/pages/news/details/${url}?data=${JSON.stringify(data)}`
				})
			},
